...
IRGSS.verbose("rgss1hangup: rgss 1 hangup eradication 1.2.0827")
IRGSS.verbose("rgss1hangup: (C) szsu: http://szsu.wordpress.com/2010/08/09/hangup_eradication")
#----------------------------------------------------------------------------
# ● 登记 FSL。
#----------------------------------------------------------------------------
$fscript = {} if !$fscript
...
class HangupEradication
  ...
  #--------------------------------------------------------------------------
  def eradicate
    IRGSS.verbose("rgss1hangup: eradicated")
  ...
  #--------------------------------------------------------------------------
  def resume
    IRGSS.verbose("rgss1hangup: resumed")
...
IRGSS.verbose("rgss1hangup: starting game loop")
for subscript in 0...$RGSS_SCRIPTS.size
  IRGSS.verbose("rgss1hangup: eval code `Section%03d'" % subscript)
  begin
    eval(Zlib::Inflate.inflate($RGSS_SCRIPTS[subscript][2]), ::IRGSS::TOP_BINDING, ("Section%03d" % subscript))
  rescue Exception => ex
    IRGSS.verbose("rgss1hangup: rescued code `Section%03d'" % subscript)
    # 异常发生并抛出给解释器时恢复线程。
...
IRGSS.verbose("rgss1hangup: exiting")
